class Item < ActiveRecord::Base
  ...
  def self.best_items(limit)
    items_by_num_orders = Item.all.sort_by{|item| item.orders.length}
    best_limit_items = items_by_num_orders.last(limit)
    best_items = []
    for i in 0..(limit - 1)
      best_items[i] = [best_limit_items.map{|item| item.name}[i], best_limit_items.map{|i| i.orders.length}[i]]
    end
    best_items
  end

  def self.best_items_inventory_and_reorder(limit)
    items_by_num_orders = Item.all.sort_by{|item| item.orders.length}
    best_limit_items = items_by_num_orders.last(limit)
    best_items_inventory_levels = []
    best_items_reorder_levels = []
    best_items_names = []
    for i in 0..(limit - 1)
      best_items_inventory_levels[i] = best_limit_items.map{|item| item.inventory_level}[i]
      best_items_reorder_levels[i] = best_limit_items.map{|item| item.reorder_level}[i]
      best_items_names[i] = best_limit_items.map{|item| item.name}[i]
    end

    [best_items_inventory_levels, best_items_reorder_levels, best_items_names]
  end
