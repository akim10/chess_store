class Order < ActiveRecord::Base
  ...
  def self.revenue_for_month(month, year)
    if (month == 0)
      month = 12
    end
    monthly_revenue = 0
    all_revenue_for_month = self.all.select{|o| o.date.month == month && o.date.year == year}
    all_revenue_for_month.each do |r|
      monthly_revenue += r.grand_total
    end
    monthly_revenue
  end
